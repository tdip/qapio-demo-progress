...
let dataSourceSettings: IDataOptions = {
    ...
    formatSettings: [{ name: 'Amount', format: 'N0' }],
    expandAll: false,
    filters: []
};
   ...
   render() {
        return (
            <div className={"bp3-dark"}>
                ...
                /*        if (!t.dataSourceSettings) {
                            return;

                        }

                        const parsed = JSON.parse(this.ref.current.getPersistData()).dataSourceSettings;

                        console.log("SETTING from", this.cellId)

                        this.props.onSaveView(this.dashboardId, {properties: {...this.props.viewProperties, xColumn: JSON.stringify({...parsed, dataSource: [], cellId: this.cellId})}, cellID: this.cellId, id: this.cellId})
                        this.props.onUpdated({json: JSON.stringify({...parsed, dataSource: [], cellId: this.cellId})})*/
                    }}
                    ...
                    displayOption={{view: 'Both'}} toolbar={[
